...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
    {
        ...
        {
            // Here we make sure the user as type somthing otherwise it will get that message
            if (user is null) return
                    new GeneralResponse(false, "Model is empty");

            //and here it chcks if user already registired
            ...
            var applicationUser = await AddToDatabase(new ApplicationUser()
            {
                ...
                //BCrypt.Net.BCrypt.HashPassword(user.Password) => we are using this to hide the password 
            });
        ...
        //And here we gonna the function FindUserByEmail it verify the user is on the database 
        ...
        public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
        {
            ...
            if (user is null) return new LoginResponse(false, "Refresh token could not be generated because user not founs");
            ...
            if (updateRefreshToken is null) return new LoginResponse(false, "Refresh token could not be generated because has not signed in");
        ...
        {
            return (await GetApplicationUsers()).FirstOrDefault(u => u.Id == id)!.Image;
        }
